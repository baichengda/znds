<template>
    <div class="l_tree_container" :class="name">
        <!-- <Icon type="ios-loading" size=13 title="刷新导航数据" class="demo-spin"
            :class="{ 'demo-spin-icon-load': afreshGetData }" @click.native.prevent="handleUpdate"></Icon> -->
        ...
        <el-menu ref="SubmenuOpen" class="build-group" :default-openeds="openNames">
            <div v-for="(build, index) in treeData" :key="index">
                ...
                <singleModelNav v-else :build="build" :treeData="build.dataList" :index="index"
...
import {http,operationManagementWs} from '@/common/request.js'
...
let handleClickOneLevel = ({ sysData = {}, build = {} } = data) => {
    ...
    if (sysData.sname === '空间管理') {
        ...
        console.log(sysData,sysData.modelId)
...
let connectWebsocket = () => {
    if (typeof WebSocket === "undefined") {
        ...
        websocket.value = !websocket.value && new WebSocket(`ws://10.243.1.200:9022/webSocketServerSensor/${user_id}`);
        // 建立连接
        ...
        websocket.value.onmessage = evt => {
            ...
            console.log(tempData,humpData,currTypeData)
            onModelLoaded(roomdbis1)
            wsdPos(devicedbids)
...
let websocketinit = () => {
    ...
    console.log(appContext.config.globalProperties,ws)
    ws.value.onopen = () => {
        console.log(realDataSendParam.value)
