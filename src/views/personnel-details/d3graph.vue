...
export default {
  ...
  data() {
    return {
      ...
      colors: ['#14AFFF', '#073AB2', '#156DFF', '#00CEF7', '#FFC0CB', '#BA55D3', '#1E90FF', '#7FFFD4', '#FFFF00'],
      states: [],
          ...
          .on('mouseenter', function (event) {
            ...
            for (let item of _this.nodes) {
              if (item.properties.name == name) {
                ...
                // _this.$set(_this.selectNodeData, 'properties', item.properties)
                _this.selectNodeData.properties = item.properties
                // 根据节点类型label获取节点颜色
                let index = 0
                switch (item.label) {
                  case _this.labels[0]:
