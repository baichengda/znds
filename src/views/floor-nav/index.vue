    ...
    <div class="l_tree_container" :class="name">
        <el-menu ref="SubmenuOpen" :default-active="activeIndex" class="build-group" :default-openeds="openNames">
            <div class="all-icon" @click="showAll">
                <img v-show="!isClickAllFloor" src="@/assets/images/floor-overview/all-icon1.png" alt="">
                <img v-show="isClickAllFloor" src="@/assets/images/floor-overview/all-icon2.png" alt="">
                <p :class="isClickAllFloor?'select-all-floor':'notclick-all-floor'">全楼</p>
            </div>
...
import { getSingleDeviceData, getStaticLabelData } from './nav-utils.js'
...
let cleanAllLabel = () => {
    ...
    showIcon([])
    window.staticIcon?.removeAllBindStaticLabel() //清除静态标签
}
...
let all = () => {
    showAllModel()
    emitter.emit('theConduit',{item: true,data:floorData})
...
let handleClickTwoLevel = ({ childSysData = {}, sysData = {} } = data) => {
    ...
    if (sysData.sname === '空间管理') { //点击楼层
        ...
        defaultView(obj[sname], true)
        
        getStaticLabelData(sname) //请求静态标签
        // getFloorCameraAngle(sname) //获取
...
let connectWebsocket = () => {
    if (typeof WebSocket === "undefined") {
        ...
        websocket.value = !websocket.value && new WebSocket( `ws://${appContext.config.globalProperties.$ws}/webSocketServerSensor/${user_id}`);
        // 建立连接
...
//         //请求成功直接用数据库保存的，如果没成功用模型中默认的
//         if (data.posX == 0 && data.posY == 0 && data.posZ == 0) {
//             // defaultView(saveAngleData.value, false);
//         } else {
//             defaultView(data, true)
//         }
//     }).catch((error) => {
...
let websocketinit = () => {
    let user_id = localStorage.getItem('user_id');
    ws.value = ws.value ? ws.value : new WebSocket('ws://' + appContext.config.globalProperties.$ws + "/websocket/JKZX" + user_id);
    ws.value.onopen = () => {
...
let websocketIcon = () => {
    let user_id = localStorage.getItem('user_id');
    wsIcon.value = wsIcon.value ? wsIcon.value : new WebSocket('ws://' + appContext.config.globalProperties.$ws + "/websocket/JKZX_ICON" + user_id);
    wsIcon.value.onopen = () => {
...
let websocketDeviceInfo = () => {
    let user_id = localStorage.getItem('user_id');
    wsInfo.value = wsInfo.value ? wsInfo.value : new WebSocket('ws://' + appContext.config.globalProperties.$ws + "/devinformation/" + user_id);
